...
def get_benchmark_subdir(scriptname):
    """Turn "2020-09-03-A-evaluation-opt-2014.py" into "2020-09-03-new14-opt"."""
    m = re.match(
        r"(?P<date>\d{4}-\d{2}-\d{2})-[A-Z]-evaluation-(?P<track>opt|sat)-(?P<year>2014|2020).py",
        scriptname,
    )
    return f"{m.group('date')}-new{m.group('year')[2:]}-{m.group('track')}"
...
BENCHMARKS = [get_benchmark_subdir(Path(SCRIPT).name)]
DOMAINS = [
...
ENVIRONMENT = BaselSlurmEnvironment(partition="infai_1", email="jendrik.seipp@unibas.ch")
