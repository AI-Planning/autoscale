...
DOMAIN_LIST = [Domain ("gripper", "gripper -n {n}",
               ...
               Domain( "woodworking", "sequential/create_woodworking_instance.py {wood_factor} {size} {num_machines} {seed}",
...
class Runner:
    ...
    def run_planners(self, parameters, time_limit=PLANNER_TIME_LIMIT):
        ...
        if result or time_limit == PLANNER_TIME_LIMIT:
            ...
            self.frontier_cache[non_linear_key].append(([parameters[linear_atr] for linear_atr in self.linear_attributes_names], result))
...
def evaluate_cfg(cfg):
    ...
    penalty = evaluate_runtimes (baseline_times) + evaluate_runtimes(sart_times)
