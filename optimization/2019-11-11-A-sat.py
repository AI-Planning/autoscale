...
DOMAINS = [
    'barman', 'blocksworld', 'childsnack', 'depots', 'driverlog', 'floortile',
    'gripper', 'hiking', 'miconic-strips', 'nomystery', 'parking', 'rover',
    'satellite', 'storage', 'tpp', 'transport', 'trucks', 'visitall',
        ...
        for nick, extra_options in [
            ...
            run.add_command(
                ...
                ['python3', os.path.join(DIR, 'linear.py'),
                 "--opt", str(SMAC_TIME_LIMIT), "--random-seed", str(seed)]
                 + extra_options + ['sat', domain] + OPTIONS,
