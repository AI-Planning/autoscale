...
for precision in [0.01, 0.001, None]:
    for domain in DOMAINS:
        for seed in range(SMAC_RUNS_PER_DOMAIN):
            run = exp.add_run()
            cmd = (["python3", os.path.join(DIR, "linear.py"),
                "--optimization-time-limit", str(SMAC_TIME_LIMIT),
                "--random-seed", str(seed),
                "--smac-output-dir", os.path.join(exp.path, f"smac-output-{domain}-{precision}")]
                + [TRACK, YEAR, domain] + OPTIONS + (["--precision", str(precision)] if precision else []))
            run.add_command(
                "optimize",
                cmd,
