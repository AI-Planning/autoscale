...
ATTRIBUTES = NUMERIC_ATTRIBUTES + [
    "error", "run_dir", "final_*", "shared_runs", "memory*"
]
...
if REMOTE:
    ...
    OPTIONS = ["--debug", "--evaluations", "1", "--tasks", "3", "--tasksbaseline", "3", "--optimization-time-limit", "10"]
    DOMAINS = ["driverlog"]
...
for domain in DOMAINS:
    for seed in range(SMAC_RUNS_PER_DOMAIN):
        ...
        run.add_command(
            "optimize",
            ["fil-profile", "run", os.path.join(DIR, "linear.py"),
