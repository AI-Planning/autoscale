...
DOMAINS = [
    "barman", "blocksworld", "childsnack",
    "floortile", "gripper", "parking",
    "scanalyzer",
...
NUMERIC_ATTRIBUTES = [
    ...
    project.Attribute("peak_memory", min_wins=True),
]
...
for challengers in ([10, 50, 100, 500, 1000, 5000, 10000] if project.REMOTE else [10]):
    for domain in DOMAINS:
        for seed in range(SMAC_RUNS_PER_DOMAIN):
            ...
            run.add_command(
                ...
                "--smac-challengers", str(challengers),
                "--smac-output-dir", os.path.join(exp.path, f"smac-output-{domain}-{challengers}")]
            ...
            problem = f"seed-{seed}"
            algorithm = f"linear-{challengers}"
...
exp.add_report(project.AverageReport(attributes=NUMERIC_ATTRIBUTES), name=f"{exp.name}-avg")
