...
void read_everything(istream &in) {
    ...
    read_operators(in);
    read_axioms(in);
    //check_magic(in, "begin_SG"); // ignore the rest
    ...
    if (g_factoring){
        ...
        if (g_leaves.empty()){
            // no factoring found, but option given to not exit
            g_factoring.reset();
        }
    }
    ...
    if (g_factoring){        
        ...
        g_leaf_factor_packers = vector<int_packer::IntPacker*>(g_leaves.size(), 0);
        for (size_t i = 0; i < g_leaf_factor_packers.size(); i++){
            g_leaf_factor_packers[i] = new int_packer::IntPacker(leaf_domains[i]);
        }
    } else {
        g_state_packer = new int_packer::IntPacker(g_variable_domain);
        g_leaf_factor_packers.push_back(new int_packer::IntPacker(vector<int>(1, 2)));
    ...
    if (g_factoring){
        for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
            cout << "Bytes per state of leaf factor " << factor << ": "
                    << g_leaf_factor_packers[factor]->get_num_bins() * sizeof(int_packer::IntPacker::Bin) << endl;
        }
    }
    ...
    if (g_factoring){
        g_leaf_successor_generators.reserve(g_leaves.size());
        for (LeafFactorID factor(0); factor < g_leaves.size(); ++factor){
            g_leaf_successor_generators.push_back(new successor_generator::SuccessorGenerator(factor));
        }
    }
...
vector<int> g_min_action_cost_per_factor;
int g_max_action_cost = 0;

// TODO adapt the size based on experiments
// also take into account non-fork factorings and new pruning methods!
size_t MAX_DUPLICATE_COUNTER = 100000;

unique_ptr<Factoring> g_factoring;
