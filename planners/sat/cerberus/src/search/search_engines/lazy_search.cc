...
//            if (check_goal_and_set_plan(current_state))
            if (check_solution_via_state_and_set_plan(node.get_real_g(), current_state, reopen))
                return SOLVED;
            if (search_progress.check_progress(current_eval_context)) {
                print_checkpoint_line(current_g);
                reward_progress();
...
bool LazySearch::check_solution_via_state_and_set_plan(int g, const GlobalState &state, bool reopen) {
    ...
    for (Heuristic *heuristic : heuristics) {
        ...
        const vector<OperatorID>& plan_from = heuristic->get_solution();
