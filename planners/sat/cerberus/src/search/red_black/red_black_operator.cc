...
void RedBlackOperator::set_black_pre_eff(const vector<bool>& black_vars) {
    ...
    unordered_map<FactPair, vector<FactProxy>> temp_cond;
...
std::vector<FactProxy> RedBlackOperator::get_red_condition(FactPair eff) const {
    std::unordered_map<FactPair, std::vector<FactProxy>>::const_iterator it = red_condition.find(eff);
...
std::vector<FactProxy> RedBlackOperator::get_black_condition(FactPair eff) const {
    std::unordered_map<FactPair, std::vector<FactProxy>>::const_iterator it = black_condition.find(eff);
...
int RedBlackOperator::get_num_red_effect_conditions(FactProxy eff) const {
    std::unordered_map<FactPair, std::vector<FactProxy>>::const_iterator it = red_condition.find(eff.get_pair());
...
bool RedBlackOperator::is_red_applicable(const int *curr_state_buffer, const unordered_set<FactPair>& extra_red_facts) const {
    for (FactProxy fact : red_precondition) {
...
bool RedBlackOperator::is_applicable(const int *curr_state_buffer, const unordered_set<FactPair>& extra_red_facts) const {
