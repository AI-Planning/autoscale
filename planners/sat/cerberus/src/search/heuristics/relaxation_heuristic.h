...
struct UnaryOperator {
    ...
    UnaryOperator(const std::vector<Proposition *> &pre, Proposition *eff, size_t effect_no_,
                  int operator_no_, int base)
        : operator_no(operator_no_), precondition(pre), effect(eff), effect_no(effect_no_),
          base_cost(base) {}
};
