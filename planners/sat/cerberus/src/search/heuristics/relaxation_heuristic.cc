...
void RelaxationHeuristic::build_unary_operators(const OperatorProxy &op, int op_no) {
    ...
    // Michael (Nov 2017): The following change comes to mimic the behavior of the old Fast Downward, where the prevails were collected first.
    /*
    for (FactProxy precondition : op.get_preconditions()) {
        precondition_props.push_back(get_proposition(precondition));
    }
    */
    vector<Proposition *> precondition_props2;
    for (FactProxy precondition : op.get_preconditions()) {
        bool is_prevail = true;
        for (EffectProxy effect : op.get_effects()) {
            if (effect.get_fact().get_variable() == precondition.get_variable()) {
                is_prevail = false;
                break;
            }
        }
        if (is_prevail)
            ...
            precondition_props2.push_back(get_proposition(precondition));
    }
    precondition_props.insert( precondition_props.end(), precondition_props2.begin(), precondition_props2.end() );
    // End of change

    for(size_t i = 0; i < op.get_effects().size(); ++i) {
        EffectProxy effect = op.get_effects()[i];
        ...
        unary_operators.push_back(UnaryOperator(precondition_props, effect_prop, i, op_no, base_cost));
