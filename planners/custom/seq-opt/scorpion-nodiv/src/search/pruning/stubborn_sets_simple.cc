...
StubbornSetsSimple::StubbornSetsSimple(double min_pruning_ratio)
    : StubbornSets(min_pruning_ratio) {
}
...
void StubbornSetsSimple::add_nes_for_unsatisfied_effect_conditions(
    int op_no, const State &state) {
    for (const FactPair &fact : sorted_op_combined_effect_conditions[op_no]) {
...
void StubbornSetsSimple::handle_stubborn_operator(const State &state,
    ...
    if (unsatisfied_precondition == FactPair::no_fact) {
        ...
        add_nes_for_unsatisfied_effect_conditions(op_no, state);
    } else {
...
static shared_ptr<PruningMethod> _parse(OptionParser &parser) {
    ...
    return make_shared<StubbornSetsSimple>(opts.get<double>("min_pruning_ratio"));
}
