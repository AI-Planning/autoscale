...
void OptionParser::add_enum_list_option(string k,
	vector<string> enumeration,
	string h, string def_val,
	vector<string> enum_docs) {
	if (help_mode_) {
	...
	//enum arguments can be given by name or by number:
	//first parse the corresponding string like a normal argument...
	add_list_option<string>(k, h, def_val);

	if (!opts.contains(k))
		return;

	auto names = opts.get_list<string>(k);
	transform(std::begin(names), std::end(names), std::begin(names), str_to_lower);

	auto enum_values = std::vector<int>();
	enum_values.reserve(names.size());
	for (const auto &name : names) {
		...
		if (!(str_stream >> x).fail()) {
			...
			enum_values.push_back(it - std::begin(enumeration));
		}
	}
	opts.set(k, enum_values);
}
