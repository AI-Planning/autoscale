...
protected:
    ...
    inline bool relaxed_task_solvable(bool level_out,
        ...
        std::vector<__gnu_cxx::hash_map<std::pair<int, int>, int, std::hash<std::pair<int, int>>> > lvl_op;
...
private:
    ...
    int loop_acyclic_graph(LandmarkNode &lmn,
                           __gnu_cxx::hash_set<LandmarkNode *, utils::Hash<LandmarkNode *>> &acyclic_node_set);
    ...
    void collect_ancestors(__gnu_cxx::hash_set<LandmarkNode *, utils::Hash<LandmarkNode *>> &result, LandmarkNode &node,
                           bool use_reasonable);
