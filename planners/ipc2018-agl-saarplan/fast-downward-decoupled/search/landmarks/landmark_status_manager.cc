...
bool LandmarkStatusManager::check_lost_landmark_children_needed_again(const LandmarkNode &node) const {
    const hash_map<LandmarkNode *, edge_type, utils::Hash<LandmarkNode *>> &children =
        node.children;

    for (hash_map<LandmarkNode *, edge_type, utils::Hash<LandmarkNode *>>::const_iterator child_it =
...
//Note: this is the same as !check_node_orders_disobeyed
    const hash_map<LandmarkNode *, edge_type, utils::Hash<LandmarkNode *>> &parents =
        node.parents;
    /*
      ...
      cout << "---------" << endl;
    */
    for (hash_map<LandmarkNode *, edge_type, utils::Hash<LandmarkNode *>>::const_iterator parent_it =
