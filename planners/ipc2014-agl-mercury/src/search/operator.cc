...
void Prevail::dump(bool dump_fact) const {
	if (dump_fact)
...
void PrePost::dump(bool dump_fact) const {
	if (dump_fact)
	    cout << (pre != -1 ? g_fact_names[var][pre] : "" ) << " => " << g_fact_names[var][post];
	else
    ...
    if (!cond.empty()) {
        ...
        for (int i = 0; i < cond.size(); i++) {
            cout << " ";
            cond[i].dump(dump_fact);
        }
    }
}

void Operator::dump(bool dump_fact) const {
    cout << name << ":";
    for (int i = 0; i < prevail.size(); i++) {
        cout << " [";
        prevail[i].dump(dump_fact);
        cout << "]";
    }
    for (int i = 0; i < pre_post.size(); i++) {
        cout << " [";
        pre_post[i].dump(dump_fact);
        cout << "]";
    }
    cout << endl;
}
