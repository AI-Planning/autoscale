...
GeneralEagerBestFirstSearchPrefOpRevisited::GeneralEagerBestFirstSearchPrefOpRevisited(
...
void
GeneralEagerBestFirstSearchPrefOpRevisited::set_pref_operator_heuristics(
    vector<Heuristic *> &heur) {
    preferred_operator_heuristics = heur;
}

void GeneralEagerBestFirstSearchPrefOpRevisited::initialize() {
...
void GeneralEagerBestFirstSearchPrefOpRevisited::statistics() const {
    search_progress.print_statistics();
    search_space.statistics();
}

int GeneralEagerBestFirstSearchPrefOpRevisited::step() {
    ...
    int count = preferred_ops.size();

    //pick count random operators
    vector<const Operator *> tmp_operators = applicable_ops;
    set<const Operator *> selected_operators;
    for (int i = count; i > 0; --i)
    {
        ...
        selected_operators.insert(to_move);
    }
    ...
    for (int i = 0; i < applicable_ops.size(); i++) {
        ...
        bool is_preferred = (selected_operators.find(op) != selected_operators.end());
...
pair<SearchNode, bool> GeneralEagerBestFirstSearchPrefOpRevisited::fetch_next_node() {
...
void GeneralEagerBestFirstSearchPrefOpRevisited::reward_progress() {
    // Boost the "preferred operator" open lists somewhat whenever
    // one of the heuristics finds a state with a new best h value.
    open_list->boost_preferred();
}

void GeneralEagerBestFirstSearchPrefOpRevisited::dump_search_space() {
    search_space.dump();
}

void GeneralEagerBestFirstSearchPrefOpRevisited::update_jump_statistic(const SearchNode &node) {
    if (f_evaluator) {
...
void GeneralEagerBestFirstSearchPrefOpRevisited::print_heuristic_values(const vector<int> &values) const {
...
SearchEngine *GeneralEagerBestFirstSearchPrefOpRevisited::create(const vector<string> &config,
                                                  int start, int &end,
                                                  bool dry_run) {
    if (config[start + 1] != "(")
    ...
    GeneralEagerBestFirstSearchPrefOpRevisited *engine = 0;
    if (!dry_run) {
        engine = new GeneralEagerBestFirstSearchPrefOpRevisited(common_options,
...
SearchEngine *GeneralEagerBestFirstSearchPrefOpRevisited::create_astar(
    ...
    if (!dry_run) {
        ...
        engine = new GeneralEagerBestFirstSearchPrefOpRevisited(
...
SearchEngine *GeneralEagerBestFirstSearchPrefOpRevisited::create_greedy(
    const vector<string> &config, int start, int &end, bool dry_run) {
    if (config[start + 1] != "(")
        throw ParseError(start + 1);
