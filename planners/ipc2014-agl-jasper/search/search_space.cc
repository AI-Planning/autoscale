...
void SearchNode::open(int h, const SearchNode &parent_node,
    ...
    if(g_h_value_recording)
        info.eval_num = g_num_h_evaluation;
}
...
void SearchSpace::trace_path(const State &goal_state,
    ...
    for (;;) {
        ...
        if (g_h_value_recording){
            eval_index.push_back(info.eval_num);
            eval_h.push_back(info.h);
        }
        if (op == 0)
            break;
        path.push_back(op);
    ...
    if(g_h_value_recording)
    {
        reverse(eval_index.begin(), eval_index.end());
        reverse(eval_h.begin(), eval_h.end());

        for (unsigned i = 0; i < eval_index.size(); ++i) {
            fprintf(g_h_value_soln_file, "%d %d\n", eval_index[i], eval_h[i]);
        }
    }
}
