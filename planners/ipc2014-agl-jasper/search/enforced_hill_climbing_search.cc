...
void EnforcedHillClimbingSearch::get_successors(const State &state, vector<const Operator *> &ops) {
    if (!use_preferred || preferred_usage == RANK_PREFERRED_FIRST) {
        ...
        if(g_rand_open && ops.size() > 1) {
            for(unsigned j = 0; j < ops.size() - 1; j++) {
                unsigned index = g_rand_gen->get_32bit_int() % (ops.size() - j);
        ...
        for (int i = 0; i < preferred_heuristics.size(); i++) {
            ...
            if(g_rand_open && preferred_ops.size() > 1) {
                for(unsigned j = 0; j < preferred_ops.size() - 1; j++) {
                    unsigned index = g_rand_gen->get_32bit_int() % (preferred_ops.size() - j);
                    const Operator *to_move = preferred_ops[j];
                    preferred_ops[j] = preferred_ops[index + j];
                    preferred_ops[index + j] = to_move;
                }
            }
