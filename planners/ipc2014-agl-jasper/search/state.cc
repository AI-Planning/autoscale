...
State::State(const State &predecessor, const MacroOperator &mo) {
    _allocate();
    State tmp_state = predecessor;
    tmp_state.borrowed_buffer = true;
    for (unsigned short i = 0; i < mo.m_path.size(); ++i)
        tmp_state = State(tmp_state, *(mo.m_path[i]));

    _copy_buffer_from_state(tmp_state);
}
