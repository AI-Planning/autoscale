...
template<class Entry, class Annotation>
void ClosedList<Entry, Annotation>::trace_path_until(
	const Entry &entry, const Entry &stop_entry, std::vector<Annotation> &path) const{
	assert(path.empty());
	Entry current_entry = entry;
	map<Entry, int> tt;
    tt[stop_entry] = 1;
	for(;;) {
		const PredecessorInfo &info = closed.find(current_entry)->second;
		if(tt.find(current_entry) != tt.end())//find already
			break;
		tt[current_entry] = 2;
		assert(info.predecessor != 0);
		path.push_back(info.annotation);
		current_entry = *info.predecessor;
	}
	reverse(path.begin(), path.end());
