...
void fix_bfmrw_configs();
void add_heuristics(BFMRW* engine);

int run_bfmrw_search(int seed) {
    ...
    fix_bfmrw_configs();
    BFMRW * engine = new BFMRW(new MTRand_int32(seed));
...
void fix_bfmrw_configs() {
    ...
    if (g_lgraph != NULL && g_lgraph->number_of_landmarks() == 0) {
        ...
        for(int i = 0; i < g_bfmrw_params_list.size(); i++) {
            if(g_bfmrw_params_list[i]->heur == MRW_Parameters::LM)
                g_bfmrw_params_list[i]->heur = MRW_Parameters::FD_FF;
        }
...
void add_heuristics(BFMRW* engine){
    ...
    for(int i = 0; i < g_bfmrw_params_list.size(); i++) {
        ...
        if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::FD_FF) {
            mrw_fd_ff_heuristic = true;
        } else if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LM) {
            mrw_landmarks_heuristic = true;
        } else if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LAMA_FF) {
            mrw_lama_ff_heuristic = true;
        } else if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LAMA_FF_S) {
            mrw_lama_ff_s_heuristic = true;
        } else if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LAMA_FF_C) {
            mrw_lama_ff_c_heuristic = true;
        } else if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::BLIND) {
    ...
    for(int i = 0; i < g_bfmrw_params_list.size(); i++) {
        if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::FD_FF)
            g_bfmrw_params_list[i]->heur_index = ff_index;
        else if(g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LM)
            g_bfmrw_params_list[i]->heur_index = landmarks_index;
        else if (g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LAMA_FF)
            g_bfmrw_params_list[i]->heur_index = lama_ff_index;
        else if (g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LAMA_FF_S)
            g_bfmrw_params_list[i]->heur_index = lama_ff_s_index;
        else if (g_bfmrw_params_list[i]->heur == BFMRW_Parameters::LAMA_FF_C)
            g_bfmrw_params_list[i]->heur_index = lama_ff_c_index;
        else if (g_bfmrw_params_list[i]->heur == BFMRW_Parameters::BLIND)
            g_bfmrw_params_list[i]->heur_index = blind_index;
    }
