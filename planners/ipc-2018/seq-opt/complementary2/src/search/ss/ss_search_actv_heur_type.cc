...
void SSSearch::probe()
{
  ...
  call_number++;
	...
	initial_value = min_h;
	cout<<"probe:"<<call_number<<",initial min_h;"<<min_h;
	...
	for(unsigned i=0;i<heuristics.size();i++){
	  active_heurs.insert(i);
	}
	cout<<"initial active_heurs"<<active_heurs.size()<<endl;
	Type type = sampler->getType(node.getId(), initial_value, 1);
	type.setActvHeurs(active_heurs);
	  ...
	  //cout<<"queue.size:"<<queue.size()<<endl;
		...
		for (size_t i = 0; i < applicable_ops.size(); ++i)
		{
			...
			//vector<int> h_child_v;
                  	boost::dynamic_bitset<> b_child_v(heuristics.size());b_child_v.reset();
#ifdef _SS_DEBUG
			cout<<"initial b_child_v:"<<b_child_v<<endl;
#endif
                  	
                  	//vector<string> heur_name_v;
			set<unsigned> active_heurs=out.getActvHeurs();
			/*  cout<<"active_heur before child,size:"<<active_heurs.size()<<",heurs:";
			for (std::set<unsigned>::iterator it=active_heurs.begin(); it!=active_heurs.end();it++){
			  cout<<*it<<",";
			}
			cout<<endl;*/
			//int max_h=0;
			for (std::set<unsigned>::iterator it=active_heurs.begin(); it!=active_heurs.end();){
			  //cout<<"working with heuristic:"<<*it<<endl;
                  	//for (size_t i = 0; i < heuristics.size(); ++i) 
                      		heuristics[*it]->evaluate(child);
				if(heuristics[*it]->is_dead_end()){
				  active_heurs.erase(it++);
				  //max_h=INT_MAX/2;
				  //cout<<"dead_end found"<<endl;
				  //h_child_v.push_back(INT_MAX/2);
				}
				else{
				  h = min(h, heuristics[*it]->get_heuristic());
				  //max_h = max(h, heuristics[*it]->get_heuristic());
#ifdef _SS_DEBUG
				  cout<<"h("<<*it<<"):"<<heuristics[*it]->get_heuristic()<<",f:"<< heuristics[*it]->get_heuristic() + g_real + get_adjusted_cost(*op)<<",thresh:"<<threshold<<endl;
#endif
				  if ( heuristics[*it]->get_heuristic() + g_real + get_adjusted_cost(*op)  > threshold) {
					active_heurs.erase(it++);
				  }
				  else{
				    b_child_v.set(*it);
			...
			//cout<<"S:"<<endl;global_state_2.dump_inline();
			   ...
			   if( queueIt != queue.end() )
			   {
				...
				if (a < prob)// &&(h+g_real+get_adjusted_cost(*op)<=threshold)) 
				{
#ifdef _SS_DEBUG
				    cout<<"\t\tAdded even though is duplicate.\n";
					  ...
					  cout<<"\t\tthe w is updated to = "<<child_node.getWeight()<<endl;
                                        ...
                                        std::pair<std::map<Type, SSNode>::iterator, bool> ret;
                                     	queue.erase(object); 
					object.setActvHeurs(active_heurs);
			   ...
			   else
			   {
			       ...
			       set<unsigned> active_heurs2=object.getActvHeurs();
			       cout<<"active_heur updated,size:"<<active_heurs2.size()<<",";
			      for (std::set<unsigned>::iterator it=active_heurs2.begin(); it!=active_heurs2.end();it++){
				cout<<*it<<",";
			      }

			       cout<<"active_heurs after evaluation:";
			      for (std::set<unsigned>::iterator it=active_heurs.begin(); it!=active_heurs.end();it++){
				cout<<*it<<",";
			      }
			      cout<<endl;
