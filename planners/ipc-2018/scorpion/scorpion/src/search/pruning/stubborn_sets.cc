...
// facts1 must be a partial state. facts2 may contain multiple facts per variable.
static bool contain_conflicting_fact_non_unique(
...
StubbornSets::StubbornSets(double min_pruning_ratio)
...
bool StubbornSets::can_disable(int op1_no, int op2_no) const {
    return contain_conflicting_fact(
        sorted_op_effects[op1_no], sorted_op_preconditions[op2_no]) ||
           contain_conflicting_fact_non_unique(
        sorted_op_effects[op1_no], sorted_op_combined_effect_conditions[op2_no]);
}
...
void StubbornSets::compute_sorted_operators(const TaskProxy &task_proxy) {
    ...
    sorted_op_combined_effect_conditions = utils::map_vector<vector<FactPair>>(
        operators, [](const OperatorProxy &op) {
            unordered_set<FactPair> conditions;
            for (EffectProxy effect : op.get_effects()) {
                for (FactProxy fact : effect.get_conditions()) {
                    conditions.insert(fact.get_pair());
                }
            }
            vector<FactPair> combined_conditions(conditions.begin(), conditions.end());
            sort(combined_conditions.begin(), combined_conditions.end());
