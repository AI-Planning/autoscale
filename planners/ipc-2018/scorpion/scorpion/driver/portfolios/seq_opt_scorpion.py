...
if has_conditional_effects(SAS_FILE):
    ...
    ABSTRACTIONS = "[projections(systematic(2), max_time=100, max_peak_memory=2048), projections(hillclimbing(max_time=100, random_seed=0)), cartesian()]"


CONFIGS = [
    (1, ["--search",
        "astar(saturated_cost_partitioning({ABSTRACTIONS}, max_time=200, max_optimization_time=2, diversify=true, random_seed=0, orders=greedy(random_seed=0)), pruning=stubborn_sets_simple(min_pruning_ratio=0.2))".format(**locals())]
    )]
