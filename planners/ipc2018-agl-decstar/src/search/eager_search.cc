...
EagerSearch::EagerSearch(
    ...
    if (opts.contains("pruning")){
        pruning_method = unique_ptr<PruningMethod>(opts.get<PruningMethod*>("pruning"));
    }
    if (g_factoring && g_factoring->get_search_type() == ASDA && opts.contains("pruning_heuristic")){
        pruning_heuristic = opts.get<Heuristic*>("pruning_heuristic");
    } else {
        pruning_heuristic = 0;
    }
    ...
    if (g_factoring && reopen_closed_nodes){
        PruningOptions::set_reopen_closed_nodes();
    }
}
...
SearchStatus EagerSearch::step() {
    ...
    if (g_factoring &&
        ...
        node.get_g() + node.get_h() >= cheapest_plan_cost){
        cout << "f-value of to be expanded node (" << node.get_g() + node.get_h()
             << ") is greater/equal to cost of best found plan => goal reached [t=" << g_timer << "]" << endl;
        return SOLVED;
    }
    ...
    for (size_t i = 0; i < applicable_ops.size(); ++i) {
        ...
        if (g_factoring){
            // g-value upper bound pruning for optimal decoupled search => g_sum_min_goal_cost
            int goal_cost = g_sum_min_goal_cost;
            if (g_factoring->get_profile() != FORK){
                // because g-value might include some goal cost
                goal_cost = max(0, goal_cost - node.get_leaf_part_g());
            }
            if ((node.get_g() + get_adjusted_cost(*op) + goal_cost) >= min(bound, cheapest_plan_cost)) {
                continue;
            }
        }
...
bool EagerSearch::insert_state(const SearchNode &succ_node) {
    ...
    if (succ_node.get_g() + succ_node.get_h() >= cheapest_plan_cost){
        return false;
    }
    ...
    // search time is significantly higher with star-heuristic upper bound pruning
...
pair<SearchNode, bool> EagerSearch::fetch_next_node() {
    ...
    while (true) {
        if (open_list->empty()) {
            if (!found_solution()){     // this happens for FDA* and AFRA* only
                cout << "Completely explored state space -- no solution! [t=" << g_timer << "]" << endl;
            } else {
                cout << "no more states in openlist [t=" << g_timer << "]" << endl;
            }
        ...
        if (g_factoring && found_solution()){
            ...
            if ((node.get_g() + max(goal_cost, node.get_h())) >= cheapest_plan_cost) {
                continue;
            }
            if (g_factoring->get_search_type() == ASDA && pruning_heuristic != 0){
                pruning_heuristic->evaluate(s);
                int fork_h = pruning_heuristic->get_heuristic();
                if (node.get_g() + fork_h >= cheapest_plan_cost){
                    continue;
                }
            }
        }
...
static SearchEngine *_parse_astar(OptionParser &parser) {
    ...
    parser.add_option<Heuristic*>("pruning_heuristic", "heuristic for upper-bound pruning in anytime decoupled search", "", OptionFlags(false));
    parser.add_option<bool>("pathmax",
                            "use pathmax correction", "false");
    parser.add_option<bool>("mpd",
                            "use multi-path dependence (LM-A*)", "false");
    parser.add_option<PruningMethod*>("pruning", 
                                      "pruning method",
                                      "",
                                      OptionFlags(false));
    SearchEngine::add_options_to_parser(parser);
