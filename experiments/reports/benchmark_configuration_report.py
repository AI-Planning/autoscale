...
class BenchmarkConfigurationReport(PlanningReport):
    ...
    def get_text(self):
        ...
        baseline_runtimes = defaultdict(list)
        sart_runtimes = defaultdict(list)
        domains = set()
        for (domain, algo), runs in sorted(self.domain_algorithm_runs.items()):
            ...
            for run in runs:
                baseline_runtimes[domain].extend(run.get("baseline_runtimes", []))
                sart_runtimes[domain].extend(run.get("sart_runtimes", []))

        result = {}
        for domain in domains:
            result[domain] = {
                "sequences": sequences[domain],
                "baseline_runtimes": baseline_runtimes[domain],
                "sart_runtimes": sart_runtimes[domain]}
